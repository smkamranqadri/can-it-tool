...
#
# Environment (all optional): MODELS, SFLAGS, LLAMA, SHIM, SHIM_PY as in run_cpu.sh, plus
...
NP=${LEVELS[-1]}
LOCK=$REPO/results/latency/mon/.cpu.lock; mkdir -p ${LOCK:h}
until mkdir $LOCK 2>/dev/null; do sleep 5; done
trap 'rmdir $LOCK 2>/dev/null' EXIT INT TERM

${LLAMA:-llama-server} -m ${MODELS:-$HOME/models}/$GGUF --host 127.0.0.1 --port 8080 \
  -c $(( NP * ${SLOT_CTX:-8192} )) -np $NP --device none -ngl 0 -t 8 --flash-attn on --jinja ${=SFLAGS:-} \
  > $MON/server.log 2>&1 &
PID=$!
...
uv run --python 3.14 python scripts/load_test.py --base-url $BASE --model $LABEL --levels $LEVELS \
  --seconds ${SECONDS_PER_LEVEL:-90} $SCEN_ARGS --out results/load/$LABEL.json 2>&1 | tee $MON/load.log
